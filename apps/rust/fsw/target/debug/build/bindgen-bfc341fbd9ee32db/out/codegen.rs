...
pub mod struct_layout {
    ...
    impl <'a, 'ctx> StructLayoutTracker<'a, 'ctx> {
        ...
        pub fn pad_struct(&mut self, layout: Layout)
         -> Option<ast::StructField> {
            debug!("pad_struct:\n\tself = {:#?}\n\tlayout = {:#?}" , self ,
                   layout);
            if layout.size < self.latest_offset {
                error!("Calculated wrong layout for {}, too more {} bytes" ,
                       self . name , self . latest_offset - layout . size);
                return None;
            }
...
use ir::template::{AsTemplateParam, TemplateInstantiation,
                   TemplateParameters};
use ir::ty::{Type, TypeKind};
use ir::var::Var;
use std::borrow::Cow;
        ...
        if let Some(val) = self.val() {
            ...
            let item =
                ast::ForeignItem{ident: ctx.rust_ident_raw(&canonical_name),
                                 ...
                                 vis: ast::Visibility::Public,};
            let item =
                ForeignModBuilder::new(abi::Abi::C).with_foreign_item(item).build(ctx);
            result.push(item);
        }
    }
}
...
impl CodeGenerator for TemplateInstantiation {
    ...
    fn codegen<'a>(&self, ctx: &BindgenContext,
                   result: &mut CodegenResult<'a>,
                   _whitelisted_items: &ItemSet, item: &Item) {
        if !ctx.options().layout_tests || self.is_opaque(ctx, item) { return }
        if ctx.uses_any_template_parameters(item.id()) { return; }
...
impl Default for AnonFieldNames {
    fn default() -> AnonFieldNames { AnonFieldNames(0) }
}
...
trait FieldCodegen<'a> {
    ...
    Extra;
    fn codegen<F,
               M>(&self, ctx: &BindgenContext, fields_should_be_private: bool,
                  accessor_kind: FieldAccessorKind, parent: &CompInfo,
                  anon_field_names: &mut AnonFieldNames,
                  result: &mut CodegenResult,
                  struct_layout: &mut StructLayoutTracker, fields: &mut F,
                  methods: &mut M, extra: Self::Extra)
    where
...
impl <'a> FieldCodegen<'a> for Field {
    ...
    fn codegen<F,
               M>(&self, ctx: &BindgenContext, fields_should_be_private: bool,
                  ...
                  result: &mut CodegenResult,
                  struct_layout: &mut StructLayoutTracker, fields: &mut F,
                  methods: &mut M, _: ()) where F: Extend<ast::StructField>,
     M: Extend<ast::ImplItem> {
        match *self {
            Field::DataMember(ref data) => {
                data.codegen(ctx, fields_should_be_private, accessor_kind,
                             parent, anon_field_names, result, struct_layout,
                             fields, methods, ());
            }
            Field::Bitfields(ref unit) => {
                unit.codegen(ctx, fields_should_be_private, accessor_kind,
                             parent, anon_field_names, result, struct_layout,
                             fields, methods, ());
            }
        }
...
impl <'a> FieldCodegen<'a> for FieldData {
    ...
    fn codegen<F,
     ...
     M: Extend<ast::ImplItem> {
        ...
        let ty = self.ty().to_rust_ty_or_opaque(ctx, &());
        let ty =
            if parent.is_union() && !ctx.options().unstable_rust {
                if ctx.options().enable_cxx_namespaces {
                    {
                        let ext_cx = &*ctx.ext_cx();
                        ::quasi::parse_ty_panic(&mut ::syntax::parse::new_parser_from_tts(ext_cx.parse_sess(),
        ...
        let field_name =
            self.name().map(|name|
                                ctx.rust_mangle(name).into_owned()).unwrap_or_else(||
                                                                                       anon_field_names.next().unwrap());
        if !parent.is_union() {
        ...
        let is_private =
            self.annotations().private_fields().unwrap_or(fields_should_be_private);
        let accessor_kind =
        ...
        if accessor_kind == FieldAccessorKind::None { return; }
...
impl BitfieldUnit {
    ...
    /// below.
    fn initial_ctor_impl(&self, ctx: &BindgenContext,
                         unit_field_int_ty: &P<ast::Ty>) -> P<ast::Item> {
            ...
            ::quasi::parse_item_panic(&mut ::syntax::parse::new_parser_from_tts(ext_cx.parse_sess(),
                                                                                {
                                                                                    ...
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::Ident(ext_cx.ident_of("impl"))));
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::Ident(ext_cx.ident_of("XxxUnused"))));
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::OpenDelim(::syntax::parse::token::Brace)));
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::Pound));
                                                                                    ...
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::CloseDelim(::syntax::parse::token::Bracket)));
                                                                                    tt.extend(::quasi::ToTokens::to_tokens(&fn_prefix,
                                                                                                                           ext_cx).into_iter());
                                                                                    tt.extend(::quasi::ToTokens::to_tokens(&ctor_name,
                                                                                                                           ext_cx).into_iter());
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::OpenDelim(::syntax::parse::token::Paren)));
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                    ...
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::RArrow));
                                                                                    tt.extend(::quasi::ToTokens::to_tokens(&unit_field_int_ty,
                                                                                                                           ext_cx).into_iter());
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::OpenDelim(::syntax::parse::token::Brace)));
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
...
impl Bitfield {
    ...
    /// bitfield unit.
    fn extend_ctor_impl(&self, ctx: &BindgenContext, parent: &CompInfo,
                        ctor_impl: P<ast::Item>, ctor_name: &ast::Ident,
                        unit_field_int_ty: &P<ast::Ty>) -> P<ast::Item> {
        let items =
        ...
        let bitfield_ty_layout =
            bitfield_ty.layout(ctx).expect("Bitfield without layout? Gah!");
            ...
            ::quasi::parse_item_panic(&mut ::syntax::parse::new_parser_from_tts(ext_cx.parse_sess(),
                                                                                {
                                                                                    ...
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::OpenDelim(::syntax::parse::token::Paren)));
                                                                                    tt.extend(::quasi::ToTokens::to_tokens(&params,
                                                                                                                           ext_cx).into_iter());
                                                                                    tt.extend(::quasi::ToTokens::to_tokens(&param_name,
                                                                                                                           ext_cx).into_iter());
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                    ...
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                    ::syntax::parse::token::OpenDelim(::syntax::parse::token::Paren)));
                                                                                    tt.extend(::quasi::ToTokens::to_tokens(&body,
                                                                                                                           ext_cx).into_iter());
                                                                                    tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
    ...
    fn codegen<F,
     ...
     M: Extend<ast::ImplItem> {
        ...
        for bf in self.bitfields() {
            bf.codegen(ctx, fields_should_be_private, accessor_kind, parent,
                       anon_field_names, result, struct_layout, fields,
                       methods,
                       (&unit_field_name, unit_field_int_ty.clone()));
            ctor_impl =
                bf.extend_ctor_impl(ctx, parent, ctor_impl, &ctor_name,
                                    &unit_field_int_ty);
        }
        match ctor_impl.unwrap().node {
...
impl <'a> FieldCodegen<'a> for Bitfield {
    ...
    (&'a str, P<ast::Ty>);
    fn codegen<F,
               M>(&self, ctx: &BindgenContext,
                  _fields_should_be_private: bool,
                  _accessor_kind: FieldAccessorKind, parent: &CompInfo,
                  _anon_field_names: &mut AnonFieldNames,
                  _result: &mut CodegenResult,
                  _struct_layout: &mut StructLayoutTracker, _fields: &mut F,
                  methods: &mut M,
                  (unit_field_name, unit_field_int_ty): (&'a str, P<ast::Ty>))
     where F: Extend<ast::StructField>, M: Extend<ast::ImplItem> {
        let prefix = ctx.trait_prefix();
                ...
                ::quasi::parse_item_panic(&mut ::syntax::parse::new_parser_from_tts(ext_cx.parse_sess(),
                                                                                    {
                                                                                        ...
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::Ident(ext_cx.ident_of("mut"))));
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::Ident(ext_cx.ident_of("unit_field_val"))));
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::Colon));
                                                                                        tt.extend(::quasi::ToTokens::to_tokens(&unit_field_int_ty,
                                                                                                                               ext_cx).into_iter());
                                                                                        ...
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::ModSep));
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::Ident(ext_cx.ident_of("ptr"))));
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::ModSep));
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::Ident(ext_cx.ident_of("copy_nonoverlapping"))));
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::OpenDelim(::syntax::parse::token::Paren)));
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::BinOp(::syntax::parse::token::And)));
                                                                                        ...
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::Dot));
                                                                                        tt.extend(::quasi::ToTokens::to_tokens(&unit_field_ident,
                                                                                                                               ext_cx).into_iter());
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                        ::syntax::parse::token::Ident(ext_cx.ident_of("as"))));
                                                                                        tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
...
impl CodeGenerator for CompInfo {
    ...
    fn codegen<'a>(&self, ctx: &BindgenContext,
                    ...
                    ::quasi::parse_item_panic(&mut ::syntax::parse::new_parser_from_tts(ext_cx.parse_sess(),
                                                                                        {
                                                                                            ...
                                                                                            tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                            ::syntax::parse::token::OpenDelim(::syntax::parse::token::Brace)));
                                                                                            tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                            ::syntax::parse::token::Ident(ext_cx.ident_of("_unused"))));
                                                                                            tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                            ::syntax::parse::token::Colon));
                                                                                            tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                            ::syntax::parse::token::OpenDelim(::syntax::parse::token::Bracket)));
        ...
        for field in self.fields() {
            field.codegen(ctx, fields_should_be_private, struct_accessor_kind,
                          self, &mut anon_field_names, result,
                          &mut struct_layout, &mut fields, &mut methods, ());
        }
        if is_union && !ctx.options().unstable_rust {
        ...
        if let Some(ref params) = used_template_params {
            for (idx, ty) in params.iter().enumerate() {
                        ...
                        ::quasi::parse_ty_panic(&mut ::syntax::parse::new_parser_from_tts(ext_cx.parse_sess(),
                                                                                          {
                                                                                              ...
                                                                                              tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                              ::syntax::parse::token::ModSep));
                                                                                              tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                              ::syntax::parse::token::Ident(ext_cx.ident_of("cell"))));
                                                                                              tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                              ::syntax::parse::token::ModSep));
                                                                                              tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                              ::syntax::parse::token::Ident(ext_cx.ident_of("UnsafeCell"))));
                                                                                              tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                              ::syntax::parse::token::Lt));
                                                                                              tt.extend(::quasi::ToTokens::to_tokens(&ident,
                                                                                                                                     ext_cx).into_iter());
                ...
                let phantom_field =
                    StructFieldBuilder::named(format!("_phantom_{}" ,
                                                      idx)).pub_().build_ty(phantom_ty);
...
impl <'a> EnumBuilder<'a> {
    ...
    fn new(aster: aster::item::ItemBuilder<aster::invoke::Identity>,
           ...
           constify: bool, constify_module: bool) -> Self {
        if bitfield_like {
            ...
            if constify_module {
                let type_definition =
                    aster::item::ItemBuilder::new().pub_().type_(CONSTIFIED_ENUM_MODULE_REPR_NAME).build_ty(repr);
                EnumBuilder::ModuleConsts{module_name: name,
                                          module_items:
                                              vec!(type_definition),}
            } else {
    ...
    fn with_variant<'b>(self, ctx: &BindgenContext, variant: &EnumVariant,
        ...
        match self {
            ...
            EnumBuilder::ModuleConsts { module_name, module_items, .. } => {
                ...
                let constant =
                    aster::AstBuilder::new().item().pub_().const_(&*variant_name).expr().build(expr).build(inside_module_type.clone());
    ...
    fn build<'b>(self, ctx: &BindgenContext, rust_ty: P<ast::Ty>,
        ...
        match self {
                        ...
                        ::quasi::parse_item_panic(&mut ::syntax::parse::new_parser_from_tts(ext_cx.parse_sess(),
                                                                                            {
                                                                                                ...
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::ModSep));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("BitOrAssign"))));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("for"))));
                                                                                                tt.extend(::quasi::ToTokens::to_tokens(&rust_ty,
                                                                                                                                       ext_cx).into_iter());
                                                                                                ...
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("fn"))));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("bitor_assign"))));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::OpenDelim(::syntax::parse::token::Paren)));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::BinOp(::syntax::parse::token::And)));
                                                                                                ...
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Comma));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("rhs"))));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Colon));
                                                                                                tt.extend(::quasi::ToTokens::to_tokens(&rust_ty,
                                                                                                                                       ext_cx).into_iter());
                        ...
                        ::quasi::parse_item_panic(&mut ::syntax::parse::new_parser_from_tts(ext_cx.parse_sess(),
                                                                                            {
                                                                                                ...
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::ModSep));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("BitAnd"))));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Lt));
                                                                                                tt.extend(::quasi::ToTokens::to_tokens(&rust_ty,
                                                                                                                                       ext_cx).into_iter());
                        ...
                        ::quasi::parse_item_panic(&mut ::syntax::parse::new_parser_from_tts(ext_cx.parse_sess(),
                                                                                            {
                                                                                                ...
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::ModSep));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("BitAndAssign"))));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("for"))));
                                                                                                tt.extend(::quasi::ToTokens::to_tokens(&rust_ty,
                                                                                                                                       ext_cx).into_iter());
                                                                                                ...
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("fn"))));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::Ident(ext_cx.ident_of("bitand_assign"))));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::OpenDelim(::syntax::parse::token::Paren)));
                                                                                                tt.push(::syntax::tokenstream::TokenTree::Token(_sp,
                                                                                                                                                ::syntax::parse::token::BinOp(::syntax::parse::token::And)));
            ...
            EnumBuilder::ModuleConsts { module_items, module_name, .. } => {
                let module_item =
                    P(ast::Item{ident: ast::Ident::from_str(module_name),
                                attrs: vec!(),
                                id: ast::DUMMY_NODE_ID,
                                node:
...
impl CodeGenerator for Enum {
    ...
    fn codegen<'a>(&self, ctx: &BindgenContext,
        ...
        fn add_constant<'a>(ctx: &BindgenContext, enum_: &Type,
                            enum_canonical_name: &str, variant_name: &str,
                            referenced_name: &str, enum_rust_ty: P<ast::Ty>,
                            result: &mut CodegenResult<'a>) {
            let constant_name =
        ...
        let mut builder =
            EnumBuilder::new(builder, &name, repr, is_bitfield,
                             is_constified_enum, is_constified_enum_module);
            ...
            match seen_values.entry(variant.val()) {
                Entry::Occupied(ref entry) => {
                    if is_rust_enum {
                        ...
                        add_constant(ctx, enum_ty, &name, &*mangled_name,
...
impl TryToOpaque for TemplateInstantiation {
    ...
    =
    Item;
    fn try_get_layout(&self, ctx: &BindgenContext, item: &Item)
     -> error::Result<Layout> {
            ...
            match self.abi() {
                ...
                Abi::Unknown(unknown_abi) => {
                    panic!("Invalid or unknown abi {:?} for function {:?} {:?}"
                           , unknown_abi , item . canonical_name ( ctx ) ,
                           self);
                }
            };
        let fnty =
...
impl CodeGenerator for Function {
    ...
    fn codegen<'a>(&self, ctx: &BindgenContext,
        ...
        let item =
            ForeignModBuilder::new(abi).with_foreign_item(foreign_item).build(ctx);
        result.push(item);
    }
}
fn objc_method_codegen(ctx: &BindgenContext, method: &ObjCMethod,
                       class_name: Option<&str>, prefix: &str)
     ...
     -> P<ast::FnDecl> {
        ...
        let decl_ty =
            signature.try_to_rust_ty(ctx,
                                     sig).expect("function signature to Rust type conversion is infallible");
